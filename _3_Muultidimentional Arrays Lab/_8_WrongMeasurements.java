            ...
            for (int c = 0; c < matrix[r].length; c++) {
                if (matrix[r][c] == wrongValue){
                    ...
                    if (isInBounds(matrix,r - 1,c) && matrix[r - 1][c] != wrongValue){
                        actualValue += matrix[r - 1][c]; //реда минус
                    }
                    //Left
                    if (isInBounds(matrix,r,c - 1)&& matrix[r][c - 1] != wrongValue){
                        actualValue += matrix[r][c - 1]; //колоната минус
                    }

                    //Down
                    if (isInBounds(matrix,r + 1,c)&& matrix[r + 1][c] != wrongValue){
                        actualValue += matrix[r + 1][c]; //реда плюс
                    }

                    //Right
                    if (isInBounds(matrix,r,c + 1)&& matrix[r][c + 1] != wrongValue){
                        actualValue += matrix[r][c + 1]; //колоната плюс
                    }
